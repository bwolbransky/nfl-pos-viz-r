...
scatter <- pbp %>%
  ...
  select(season, passer, is_home, week, team_spread, yardline_100, yards_gained, complete_pass, epa, total_line, touchdown, interception, winner) %>%
  group_by(passer, winner, season,is_home, week, total_line,team_spread) %>%
  summarize(yds_gained = sum(yards_gained), tds = sum(touchdown), comps = sum(complete_pass), ints = sum(interception)) %>%
  group_by(passer, season, is_home,team_spread, total_line, winner)  
...
scatter %>%
  ...
  geom_jitter(height = 0.00, width = 0.00, cex=2)+
  ...
  geom_vline(aes(xintercept = total_line), linetype = "dashed", color = "grey")
