...
scatter <- df %>%
  ...
  filter(defteam == def, !is.na(epa), complete_pass == 1, season >= 2019) %>%
  select(receiver_id, season, game_id, receiver, is_home, week, team_spread, yardline_100, yards_gained, epa, spread_line, total_line, touchdown, winner) %>%
  group_by(receiver_id, game_id, season, receiver, is_home, week, team_spread, total_line, winner) %>%
  summarize(yds_gained = sum(yards_gained), tds = sum(touchdown)) %>%
  nflfastR::decode_player_ids()

scatter <- scatter %>%
  merge(roster[,c("position", "gsis_id", "season")], by.x=c('receiver_id',"season"), by.y= c('gsis_id',"season")) %>%
  filter(position == poz) %>%
  group_by(game_id) %>%
  top_n(2, yds_gained)
...
scatter %>%
  ...
  geom_point(cex = 3)+
  annotate(geom = "text",x=scatter$team_spread,y=scatter$yds_gained, label = scatter$receiver, cex = 2.5) +
