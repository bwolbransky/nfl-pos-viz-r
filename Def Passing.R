...
scatter <- pbp %>%
  ...
  filter(defteam == def, !is.na(epa), complete_pass == 1, season >= 2018) %>%
  select(season, game_id, passer, is_home, week, team_spread, yardline_100, yards_gained, epa, spread_line, total_line, touchdown, winner) %>%
  group_by(game_id, season, passer, is_home, week, team_spread, total_line, winner) %>%
...
scatter %>%
  ...
  theme_bw() +
  labs(x= "Vegas Spread", y = "Passing Yards", title = paste(def, "Defensive Yards Allowed"), subtitle = paste("Home vs Away Splits since",min(scatter$season)), caption = 
