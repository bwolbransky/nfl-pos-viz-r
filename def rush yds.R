...
scatter <- df %>%
  ...
  filter(defteam == def, !is.na(epa), rush == 1, season >= 2000) %>%
  select(rusher_id, season, game_id, rusher, is_home, week, team_spread, yardline_100, yards_gained, epa, spread_line, total_line, touchdown, winner) %>%
  group_by(rusher_id, game_id, season, rusher, is_home, week, team_spread, total_line, winner) %>%
  summarize(yds_gained = sum(yards_gained), tds = sum(touchdown)) %>%
  nflfastR::decode_player_ids()

scatter <- scatter %>%
  merge(roster[,c("position", "gsis_id")], by.x='rusher_id', by.y= 'gsis_id') %>%
  filter(position == poz) %>%
  group_by(game_id)%>%
...
scatter %>%
  ...
  geom_point(cex = 3)+
  annotate(geom = "text",x=scatter$team_spread,y=scatter$yds_gained, label = scatter$rusher, cex = 2.5) +
